            ...
            <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !border-b-DEFAULT`}>
              <TableCell className="max-w-[250px] pl-2 pr-10">
                <div className="flex items-center gap-3">
                  {t.image ? (
                    <Image alt={'image'} src={`${t.image || ''}`} className="w-[30px] h-[30px] px-0 mx-0 rounded-full" width={30} height={30} />
                  ) : (
                    <div className=" w-[30px] h-[30px] px-0 mx-0 rounded-full flex p-0 m-0 bg-gray-400 justify-center align-middle">
                      <p className=" h-auto p-0 pt-1 transaction-image text-center flex  font-semibold   align-center align-middle">
