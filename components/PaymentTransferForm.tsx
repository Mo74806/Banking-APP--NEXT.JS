...
const PaymentTransferForm = ({ accounts }: PaymentTransferFormProps) => {
  ...
  const submit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (transfer) {
        ...
        if (newTransaction) {
          ...
          toast({
            ...
            description: 'Your Transaction Done Successfully',
          });
          router.push('/');
        }
      }
      ...
      setError('There an invalid data Please verify yur data and try again');
    }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submit)} className="flex flex-col">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="border-t border-gray-200">
              <div className="payment-transfer_form-item pb-6 pt-5">
                <div className="payment-transfer_form-content">
                  <FormLabel className="text-14 font-medium text-gray-700">Name</FormLabel>
                  <FormDescription className="text-12 font-normal text-gray-600">Name will appear at the Recipient Account</FormDescription>
                </div>
                <div className="flex w-full flex-col">
                  <FormControl>
        ...
        <CustomInput1 schema={formSchema} control={form.control} name="amount" label="Amount" placeholder="ex: 5.00" />
        {/* {Boolean(Number(form?.amount)) && <p className="text-red-500 text-[16px] font-semibold">must be a number</p>} */}
