...
const RecentTransactions = ({ accounts, transactions = [], appwriteItemId, page = 1 }: RecentTransactionsProps) => {
  ...
  return (
    <section className="recent-transactions  !w-full ">
      <header className="flex items-center  justify-betweenW w-[100%]">
        <h2 className=" inline recent-transactions-label">Recent transactions</h2>
        <Link href={`/transaction-history/?id=${appwriteItemId}`} className="view-all-btn inline ms-auto">
          View all
        </Link>
      </header>
